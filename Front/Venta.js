...
$(document).on('keyup','[id=fechaVencimiento]',function(e){
...
function obtenerTarjetas(id){
  ...
  fetch("http://185.254.205.224:60227/api/Tarjetas/user/"+id, requestOptions)
    ...
    result.tarjetas.forEach(element => {
      ...
      element.fechaVencimiento = element.fechaVencimiento.toString().substring(0,2)+"/"+element.fechaVencimiento.toString().substring(2)
...
$( "#ventaForm" ).submit(function( event ) {
  ...
  if($('#medioDepago').val()!=='EFECTIVO'){
    VentaRequest.tarjetaClie.fechaVencimiento = $("#fechaVencimiento").val()
...
let mes = objetoRequest.tarjetaClie.fechaVencimiento.toString().substring(0,2)
let anio = objetoRequest.tarjetaClie.fechaVencimiento.toString().substring(3)
objetoRequest.tarjetaClie.fechaVencimiento =  parseInt("0"+mes+anio,10);
